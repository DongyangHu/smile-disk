  ...
  $.i18n.properties = function (settings) {
    ...
    var languagesFileLoadedCallback = function (languages) {
      ...
      for (var k = 0, m = files.length; k < m; k++) {
        /*2016年04月06日 20-47-01 add by luogj 因项目要求，语言种类必须有，且为zh_HK之类，不存在zh文件，此处不需要在
         判断默认文件和shortcode了，避免控制台总是输出无效的404错误信息
        // 1. load base (eg, Messages.properties)
        loadAndParseFile(settings.path + files[k] + settings.suffix, settings);
        // 2. with language code (eg, Messages_pt.properties)
        var shortCode = settings.language.substring(0, 2);
        if (languages.length == 0 || $.inArray(shortCode, languages) != -1) {
          loadAndParseFile(settings.path + files[k] + '_' + shortCode + settings.suffix, settings);
        }*/
        // 3. with language code and country code (eg, Messages_pt_BR.properties)
        if (settings.language.length >= 5) {
          var longCode = settings.language.substring(0, 5);
          if (languages.length == 0 || $.inArray(longCode, languages) != -1) {
            loadAndParseFile(settings.path + files[k] + '_' + longCode + settings.suffix, settings);
          }
        }
      }
